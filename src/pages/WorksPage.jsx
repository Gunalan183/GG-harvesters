...
const WORK_IMAGES = [
  { image: '/images/works/work02.png',  alt: 'GG Harvesters field work — நெல் அறுவடை பணி', category: 'harvester' },
  { image: '/images/works/Work01.jpeg', alt: 'GG Harvesters JCB work — JCB பணி',            category: 'jcb' },
  { image: '/images/works/work03.jpg',  alt: 'GG Harvesters JCB work — JCB பணி',            category: 'jcb' },
  { image: '/images/works/Work04.jpg',  alt: 'GG tractor with tailer transport work',         category: 'tractor' },
  { image: '/images/works/work05.png',  alt: 'GG tractor rotavator land preparation work',   category: 'tractor' },
  { image: '/images/works/work06.png',  alt: 'GG lorry transport service work',               category: 'lorry' },
];
...
export default function WorksPage() {
  ...
  const t = getTranslations(lang).worksPage;

  // Build works array by merging static image data with translated strings
    ...
    <div className="flex flex-col min-h-screen font-sans" style={{ background: '#fff' }}>
      ...
      <main className="flex-1 pt-[68px] pb-24">
        ...
        >
          ...
          >
            {t.heading}{' '}
            <span style={{ color: '#f0cc5a' }}>{t.headingHighlight}</span>
          </h1>
          <p className="text-white/60" style={{ fontSize: '0.95rem', maxWidth: '460px', lineHeight: 1.7 }}>
            {t.subtitle}
          </p>
          ...
          <div className="flex gap-8 mt-8" style={{ flexWrap: 'wrap', justifyContent: 'center' }}>
            {t.stats.map((s) => (
              <div key={s.label} className="text-center">
                <p className="font-extrabold" style={{ fontSize: 'clamp(1.6rem, 5vw, 2rem)', color: '#f0cc5a', lineHeight: 1 }}>{s.num}</p>
        ...
        <div style={{ padding: '2.5rem 1rem' }}>
          ...
          >
            ...
            <span
              className="absolute top-4 left-4 text-xs font-bold rounded-full px-3 py-1"
            ...
            <div className="absolute bottom-5 left-5 right-5">
              <h2 className="text-white font-extrabold" style={{ fontSize: 'clamp(1.1rem, 4vw, 1.4rem)', lineHeight: 1.2 }}>
                {WORKS[0].title}
              </h2>
              <p className="text-white/65 text-xs mt-1 leading-relaxed line-clamp-2">{WORKS[0].desc}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {WORKS.slice(1).map((work, i) => (
              ...
              >
                ...
                <span
                  className="absolute top-3 left-3 text-[0.6rem] font-bold rounded-full px-2 py-0.5"
                ...
                <div className="absolute bottom-3 left-3 right-3">
                  <h3 className="text-white font-bold text-sm leading-snug">{work.title}</h3>
                </div>
              </div>
            ))}
          </div>
        ...
        >
          <p className="text-white/60 text-xs uppercase tracking-widest font-bold mb-2">{t.ctaBadge}</p>
          <h3 className="text-white font-extrabold mb-1" style={{ fontSize: 'clamp(1.1rem, 4vw, 1.4rem)' }}>
            {t.ctaHeading}
          </h3>
          <p className="text-white/55 text-sm mb-4">{t.ctaSubtitle}</p>
          <a
            href={whatsappLink(WHATSAPP_MESSAGES.general)}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 font-bold rounded-full active:scale-95 transition-transform"
      ...
      {lightbox !== null && (
        ...
        >
          ...
          <button onClick={(e) => { e.stopPropagation(); prev(); }}
            className="absolute left-3 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full flex items-center justify-center z-10"
          ...
          <div className="relative max-w-2xl w-full px-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 text-center">
              <span
                className="inline-block text-xs font-bold rounded-full px-3 py-1 mb-2"
                style={{ background: TAG_COLORS[WORKS[lightbox].category].bg, color: TAG_COLORS[WORKS[lightbox].category].text }}
              >
                {WORKS[lightbox].tag}
              </span>
              <h3 className="text-white font-extrabold text-lg">{WORKS[lightbox].title}</h3>
              <p className="text-white/60 text-sm mt-1 leading-relaxed">{WORKS[lightbox].desc}</p>
              <p className="text-white/30 text-xs mt-3">{lightbox + 1} / {WORKS.length}</p>
            </div>
          </div>

          <button onClick={(e) => { e.stopPropagation(); next(); }}
            className="absolute right-3 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full flex items-center justify-center z-10"
